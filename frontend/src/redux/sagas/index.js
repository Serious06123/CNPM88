...
function* createHisBookActionSaga(action) {
    try {
        let data = yield call(api.createHisBook, action.payload);


        yield put(actions.createHisBook.createBookSuccess(data.data.errcode))

    } catch (err) {
        console.log(err);
        yield put(actions.createHisBook.createBookFail(err))
    }
}
...
function* hisBookActionSaga(action) {
    try {
        const datahisbook = yield call(api.hisBook, action.payload);

        yield put(actions.hisBook.hisBookSuccess(datahisbook.data.data))

    } catch (err) {
        console.log(err);
        yield put(actions.hisBook.hisBookFail(err))
    }
}
...
function* mySaga() {
    ...
    yield takeLatest(actions.createHisBook.createBookRequest, createBookActionSaga)
    ...
    yield takeLatest(actions.hisBook.hisBookRequest, hisBookActionSaga)
